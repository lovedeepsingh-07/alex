    ...
    match response {
        protocol::Response::OK(data) => {
            match cli_args.sub_command {
                ...
                cli::SubCommand::Play { audio_label: _ } => {
                    match data {
                        Some(audio_label) => println!("> Playing {}", audio_label),
                        None => {},
                    }
                },
                ...
                cli::SubCommand::Resume => {
                    println!("> Resuming playback");
                },
                cli::SubCommand::Clear => {
                    println!("> Clearing player queue");
                },
                _ => {
                    return Err(error::Error::ProtocolError(format!("Something went wrong!")));
                }
            }
        },
        ...
        protocol::Response::StatusData(status_data) => handle_status_response(cli_args, status_data)?,
...
fn handle_status_response(cli_args: cli::CliArgs, status_data: protocol::StatusData) -> Result<(), error::Error> {
    if let cli::SubCommand::Status { sub_command } = cli_args.sub_command {
        match sub_command {
            Some(cli::StatusSubCommand::CurrentAudio) => {
                match (status_data.current_audio, cli_args.just_info) {
                    ...
                    (Some(current_audio), false) => println!("> Current Audio: {}", current_audio.purple()),
                    (None, true) => print!("NO AUDIO"),
                    (None, false) => println!("> No audio is playing"),
                }
            },
            Some(cli::StatusSubCommand::IsPaused) => {
                match cli_args.just_info {
                    true => print!("{}", status_data.is_paused),
                    false => println!("> {}", status_data.is_paused),
                }
            },
            Some(cli::StatusSubCommand::IsQueueEmpty) => {
                match cli_args.just_info {
        ...
        return Err(error::Error::ProtocolError("Request and Response structure do not match".to_string()));
    }
